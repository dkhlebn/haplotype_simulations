...
def create_mock_haps(chrom, chrom_sectors):
    ...
    mock_f = ps.VariantFile(
        f"{ARGS.wd}/{chrom}/1kg_haps.merged.vcf.gz", "w", header=merged.header
    )
    ...
    create_tabix_index(f"{ARGS.wd}/{chrom}/1kg_haps.merged.vcf.gz")
    return f"{ARGS.wd}/{chrom}/1kg_haps.merged.vcf.gz"
...
def execute_data_svd(plink_fs_prefix, n_cmp=5):
    ...
    sample_ids = pd.read_csv(f"{plink_fs_prefix}.mdist.id", header=None, names=["famids", "IID"])
    ...
    res = pca_df.merge(ARGS.imprinting_data, left_index=True, right_on="Sample").set_index("Sample")
    return res
...
def run_step(chr_dt):
    ...
    args = [{"chrom": chrom, "sectors": sectors, "args": ARGS}
                         for chrom, sectors in sectors.items()]
    chr2pref = pqdm_p(args, run_mocking, n_jobs=22, argument_type="kwargs")
    args = [{"chrom": t[0], "plink_pref": t[1], "args": ARGS} for t in chr2pref]
    step_results = pqdm_t(
